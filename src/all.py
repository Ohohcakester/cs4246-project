...
for index, row in df.iterrows():
    if index != 0 and row.Z == df.iloc[index - 1].Z:
        if row.Z == 0:
            ...
            prev_coordinate = floor2grid.queryActual(df.iloc[index-1].X,
                                                     df.iloc[index-1].Y)
            df_pairs_floor2 = df_pairs_floor2.append({'timestamp': row.SNAPSHOT_TIMESTAMP,
                                                      'Z': row.Z, 'user': user,
                                                      'pair': (prev_coordinate + current_coordinate)},
                                                     ignore_index=True)
        else:
            ...
            df_pairs_floor18 = df_pairs_floor18.append({'timestamp': row.SNAPSHOT_TIMESTAMP,
                                                        'Z': row.Z, 'user': user,
                                                        'pair': (prev_coordinate + current_coordinate)},
                                                       ignore_index=True)
