...
There are many advantages to using the Gaussian Process model. One key property of the data is that it is unevenly sampled, with multiple large gaps in between the position readings $(x,y,z)$ of each individual. As these points are discrete and finite, the crowd densities at certain positions and times are not represented by the data. After processing the data with using the Gaussian Process model, we will be able to fill in these gaps in data, allowing us to compute the proportion of the crowd at any point of time, over any defined region. \\

The model is also useful because of the correlation between the current positions



There have been past usages of the GP model trajectory prediction in data. For example, Gaussian Processes were used to model the trajectory of human vital signs...
