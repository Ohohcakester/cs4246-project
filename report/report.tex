...
The rationale behind the choice of Gaussian process for modelling of crowds as by prediction the population density within a defined radius of a certain point, we can do better crowd control. For example, it will be useful in the case of train breakdowns to facilitate crowd movements. In addition, we can perform a certain form of path prediction using the GP model. \\
...
The use of the GP model in the application enables us to use all the samples and feature information to perform the prediction, while allowing the training data to have different or uneven sampling rates. Particularly, in our dataset, not all the participants have their position data sampled at the same timestamps, and all of them might not have the same number of sample points. \\
